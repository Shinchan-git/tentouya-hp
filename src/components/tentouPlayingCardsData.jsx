...
import "../css/tentouPlayingCards.css"

const tentouPlayingCardsData = [
  {
    number: "A",
    mark: "spade",
    name: "ナミテントウ",
...
export default function TentouCardsData() {
  let tentouCardsData = []
  const data = tentouPlayingCardsData

  for (let i = 0; i < data.length; i++) {
    const number = data[i].number
    const mark = markHtml(data[i].mark)
    const scientificNameItalic = data[i].scientificNameItalic
    const scientificNameUpright = data[i].scientificNameUpright
    const rarity = data[i].rarity
    const description = data[i].description

    function Star(props) {
      if (props.rarity === "tohoshi") {
        const tohoshiStar = "\u2605\u2605（関西では\u26054）"
        return <span>{tohoshiStar}</span>
      } else {
        let star = ""
        for (let j = 0; j < props.rarity; j++) {
          star += "\u2605"
        }
    ...
    function ScientificNameItalic(props) {
      if (props.text === "Sticholotis sp.") {
        return (
          <React.Fragment>
            <span className="scientific-name-italic">Sticholotis</span>&nbsp;<span className="scientific-name-upright">sp.</span>
          </React.Fragment>
        )
      } else {
        return <span className="scientific-name-italic">{props.text}</span>
      }
    }

    function ScientificNameUpright(props) {
      if (props.text === "未記載") {
        return <span className="main-text">{props.text}</span>
      } else {
        return <span className="scientific-name-upright">{props.text}</span>
      }
    }

    tentouCardsData.push(
      <div className="tentou-cards-data">
        <p className="main-text">
          ...
          <span className="inline-block">
            <ScientificNameItalic text={scientificNameItalic} />&nbsp;
            <ScientificNameUpright text={scientificNameUpright} />&nbsp;&nbsp;
          </span>
          <Star rarity={rarity} />
        </p>
        <p className="main-text tentou-cards-data-description">
          {description}
        </p>
      </div>
    )
